...
class AsyncAkshareDataUpdater(AsyncBaseDataUpdater):
    ...
    async def _get_stock_list(self) -> list:
        ...
        stock_list_df = await asyncio.get_event_loop().run_in_executor(
            None, func=ak.stock_zh_a_spot_em
        )
    ...
    async def _get_existing_data_range(self, stock_code: str, collection_name: str):
        ...
        earliest_record = await collection.find_one(
            {"orderbook_id": stock_code}, sort=[("timestamp", 1)]
        )
        ...
        logger.debug(
            f"Existing data range for {stock_code} in {collection_name}: {earliest} - {latest}"
        )
    ...
    def download_data(self, stock_code, freq, start_date, end_date) -> pd.DataFrame:
        logger.info(f"Downloading {stock_code} {freq} data...")
        if freq == "1m":
    ...
    async def fetch_and_process_data(self, stock_code, freq):
        ...
        def fetch_data():
            if earliest is not None and latest is not None:
                newer = self.download_data(stock_code, freq, start_date=latest, end_date="20500101")
                older = self.download_data(stock_code, freq, start_date="19700101", end_date=earliest)
                return pd.concat([older, newer])
            else:
                return self.download_data(stock_code, freq, start_date="19700101", end_date="20500101")
...
if __name__ == "__main__":
    ...
    updater = AsyncAkshareDataUpdater(
        db_name="cn_stock", bar_sizes=["1m", "1D", "1W", "1M"], client_str=client_str
    )
